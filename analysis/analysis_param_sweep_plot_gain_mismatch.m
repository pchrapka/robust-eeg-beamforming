function analysis_param_sweep_plot_gain_mismatch(cfg)
%ANALYSIS_PARAM_SWEEP_PLOT_GAIN_MISMATCH
%   ANALYSIS_PARAM_SWEEP_PLOT_GAIN_MISMATCH(CFG) plots the gain of the filter
%   for a mismatched leadfield matrix
%
%   Input
...
for i=1:length(cfg.file_in)
    ...
    snr = unique([out.snr]);
    if length(snr) > 1
        warning('reb:analysis_param_sweep_plot_gain_mismatch',...
...
figure;
plot_series(data);
title('Beampattern for mismatched leadfield matrix');
