...
addParameter(p,'signal_idx',[],@isnumeric);
addParameter(p,'interference_idx',[],@isnumeric);
...
if isempty(p.Results.signal_idx) || isempty(p.Results.interference_idx)
    error('missing signal_idx or interference_idx');
end
...
for i=1:length(params)
    ...
    for j=1:length(params(i).beamformer_configs)
        for k=1:length(p.Results.snrs)
            ...
            metrics{2}.location_idx = p.Results.signal_idx;
            metrics{3} = metrics_avg;
            metrics{3}.name = 'snr-beamformer-output';
            metrics{3}.location_idx = p.Results.signal_idx;
            metrics{4} = metrics_avg;
            metrics{4}.name = 'isnr-beamformer-output';
            metrics{4}.location_idx = p.Results.interference_idx;
