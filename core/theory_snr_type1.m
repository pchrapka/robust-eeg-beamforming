function theory_snr_type1(source,sim,snr,vertex)
...
if nsources > 1
    ...
    fprintf('Input SNR: %0.2f, %0.2f dB\n',alpha,db(alpha,'power'));
    
    omega = alpha/(1+alpha);
    
    if isempty(p.Results.BeamformerVertex)
        % do something generic
    else
        if p.Results.VarNoise ~= 1
            error('formulas assume noise variance is 1');
        end
        ...
        fprintf('Output SNR approx: %0.2f, %0.2f dB\n',snr_approx,db(snr_approx,'power'));
        fprintf('SNR factor: %0.2f\n',(snr_approx-1)/alpha);
